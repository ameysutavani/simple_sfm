    ...
    GIVEN("A test BAL file stream")
    {
        ...
        WHEN("Calling loadFromBALFileStream on the file stream")
        {
            ...
            THEN("The SfmProblem should be loaded correctly")
            {
                ...
                // Check the loaded data matches that of the test file for a few entries.
                // Check parameters of the second observation.
                ...
                REQUIRE(observation.pixel[1] == Approx(-5.229800e+02).margin(1e-6)); // Inverted v coordinate.
                ...
                */
                ...
                // TODO: transform to world frame and from OpenGL to GTSAM (like OpenCV) convention.
                REQUIRE(camera.axis_angle[0] == Approx(-3.4265630475549310e-03).margin(1e-6));
                REQUIRE(camera.axis_angle[1] == Approx(9.6448984178004217e-03).margin(1e-6));
                REQUIRE(camera.axis_angle[2] == Approx(4.6562227695769124e-03).margin(1e-6));
                REQUIRE(camera.translation[0] == Approx(8.2039923232261648e-03).margin(1e-6));
                REQUIRE(camera.translation[1] == Approx(4.1758870714924805e-04).margin(1e-6));
                REQUIRE(camera.translation[2] == Approx(1.0139126763172436e-01).margin(1e-6));
                REQUIRE(camera.focal_length == Approx(2.8443148232166736e+03).margin(1e-6));
                REQUIRE(camera.distortion_k1 == Approx(-2.0200951857532239e-08).margin(1e-6));
                REQUIRE(camera.distortion_k2 == Approx(2.1246398231307891e-15).margin(1e-6));
