...
namespace simple_sfm
{
    namespace utils
    {
        SfmProblem<> loadFromBALFileStream(std::ifstream &file)
        {
            ...
            // Get the number of cameras, points, and observations from the first line of the file.
            ...
            {
                file >> observation.camera_index >> observation.point_index >> observation.pixel[0] >> observation.pixel[1];
                observation.pixel[1] = -observation.pixel[1]; // Invert v coordinate to transform from OpenGL to GTSAM (like OpenCV) convention.
            }
            ...
            for (auto &camera : problem.variables.cameras)
            {
                ...
                file >> camera.axis_angle[0] >> camera.axis_angle[1] >> camera.axis_angle[2];
                file >> camera.translation[0] >> camera.translation[1] >> camera.translation[2];
